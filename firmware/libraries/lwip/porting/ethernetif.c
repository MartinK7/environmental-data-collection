...
{
	static BaseType_t yield;
	yield = pdFALSE;
	if(xPortIsInsideInterrupt()) {
		if(xSemaphoreGiveFromISR(RxPktSemaphore, &yield) == pdTRUE)
...
static void low_level_init(struct netif *netif)
{
  ...
  gpio_pin_write(GPIO_PIN_ETH_EN, GPIO_PIN_STATE_LOW);
  vTaskDelay(10 / portTICK_PERIOD_MS);
  gpio_pin_write(GPIO_PIN_ETH_EN, GPIO_PIN_STATE_HIGH);
  ...
  xTaskCreateStatic(ethernetif_input, "EthIf", STACK_SIZE, (void *)netif, 1, xStack, &xTaskBuffer);
...
static struct pbuf * low_level_input(struct netif *netif)
{
  ...
  if (HAL_ETH_GetRxDataBuffer(&heth, RxBuff) == HAL_OK)
  {
	...
	#warning "This line was commented out!! Use here MPU probabraly"
